...
import userGuidesFakerFactory from "./userGuidesFakerFactory";
...
const UserGuidesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userGuidesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userGuides").create(elem));
...
export default connect(mapState, mapDispatch)(UserGuidesFakerDialogComponent);
